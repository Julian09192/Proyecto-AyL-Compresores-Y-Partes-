...
import { supabase } from "../lib/client"; // Conexión directa a Supabase

function ProfilePage({ usuario: usuarioProp, setVista, volverA, onUsuarioUpdate }) {
  ...
  const [datosForm, setDatosForm] = useState({
    ...
    ciudad: "",
    usuario: "",
    num_identificacion: "", // Consistencia con perfiles Admin/Empleado
    rol: "cliente"
  });

  // Cargar información real directamente desde Supabase Auth y BD remota
  const cargarUsuarioAutenticado = async () => {
    try {
      ...
      // 2. Consultar directamente la tabla de usuarios
      ...
      if (perfilData) {
        setDatosForm({
          id: perfilData.id || "",
          nombre: perfilData.nombre || perfilData.usuario || "Usuario",
          email: perfilData.correo || user.email || "correo@ejemplo.com",
      ...
      console.error("Error cargando perfil autónomo:", error.message);
      
      // Respaldo secundario por si falla la conexión directa, usando las props heredadas
      if (usuarioProp) {
        setDatosForm({
          id: usuarioProp.id || "",
          nombre: usuarioProp.nombre || usuarioProp.usuario || "Usuario",
          email: usuarioProp.email || "correo@ejemplo.com",
          telefono: usuarioProp.telefono || "",
          ciudad: usuarioProp.ciudad || "",
          usuario: usuarioProp.usuario || "",
          num_identificacion: usuarioProp.num_identificacion || "",
          rol: usuarioProp.rol || "cliente"
        });
      }
    } finally {
  ...
  const handleCancelar = () => {
    cargarUsuarioAutenticado(); // Revierte el formulario trayendo de nuevo los datos guardados en Supabase
    setEditando(false);
  };
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      // Guardar cambios directamente en Supabase, ignorando la API local rota
      const { data, error } = await supabase
        .from('usuario')
        .update({
          nombre: datosForm.nombre,
          num_celular: datosForm.telefono || null,
          ciudad: datosForm.ciudad || null,
          num_identificacion: datosForm.num_identificacion || null,
      ...
      Swal.fire({
        ...
        text: "Tu información se ha modificado con éxito.",
      ...
      const usuarioActualizado = {
        ...
        num_identificacion: datosForm.num_identificacion,
        usuario: datosForm.usuario,
        rol: datosForm.rol
      };

      // Sincronizar el LocalStorage local para mantener las persistencias de la interfaz de usuario
  ...
  const mostrarBotonDashboard = datosForm.rol && ["admin", "empleado"].includes(datosForm.rol);
  const vistaDashboard = datosForm.rol === "admin" ? "admin" : "empleado";
  ...
  return (
    <div className="bg-light min-vh-100 seccion-perfil-contenedor">
      <div className="container container-max-width">
        ...
        {/* Header superior */}
        <div className="d-flex flex-column flex-sm-row align-items-start align-items-sm-center justify-content-between mb-4 mb-md-5 gap-3">
              ...
              <div className="pt-2 w-100 border-top border-light">
                <div className="row g-2 text-start mt-2">
                  ...
                  <div className="col-6">
                    ...
                    <span className="fw-bold text-secondary small text-truncate d-block">
          ...
          <div className="col-lg-8">
            <div className="card border-0 shadow-sm p-4 rounded-4 bg-white">
              ...
              <div className="row g-3">
                ...
                <div className="col-md-6">
                  ...
                  <div className="input-group">
                    ...
                    <input
                      type="text"
                      name="nombre"
                      className={`form-control form-control-custom ${editando ? 'bg-white border-warning' : 'bg-light-disabled'}`}
                ...
                {/* Número de Identificación (¡Campo habilitado!) */}
                <div className="col-md-6">
                  <label className="form-label-custom">Número de Identificación</label>
                  <div className="input-group">
                    <span className="input-group-text bg-light border-end-0 text-muted">
                    ...
                    <input
                      ...
                      className={`form-control form-control-custom ${editando ? 'bg-white border-warning' : 'bg-light-disabled'}`}
                      value={datosForm.num_identificacion || ""}
                      onChange={handleChange}
                      disabled={!editando}
