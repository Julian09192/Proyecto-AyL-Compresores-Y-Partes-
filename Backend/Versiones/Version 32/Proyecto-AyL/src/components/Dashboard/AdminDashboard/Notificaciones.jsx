...
export default function NotificacionesView() {
  ...
  const optimizarImagenCloudinary = (url) => {
    ...
    if (url.includes("res.cloudinary.com")) {
      // Cambiado a c_limit para que preserve la proporción original en lugar de forzar un fill
      return url.replace("/upload/", "/upload/f_auto,q_auto,w_110,h_110,c_limit/");
    }
    return url;
  };
...
const styles = {
  ...
  // Se corrigió 'justifycontent' a 'justifyContent' y se añadió padding interno para que respire la imagen
  imgWrapper: { backgroundColor: '#f7fafc', border: '1px solid #edf2f7', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden', width: '56px', height: '56px', padding: '4px', boxSizing: 'border-box' },
  // Se cambió 'cover' por 'contain' para que el aceite/repuesto mantenga su aspecto original sin estirarse
