...
function Productos() {
    ...
    const registrarEventoProducto = (producto, accion, detalle, cambios = []) => {
        const nuevoRegistro = {
            id: `${Date.now()}-${producto.id || "nuevo"}`,
            accion,
        ...
        try {
            const registros = JSON.parse(localStorage.getItem(BITACORA_PRODUCTOS_KEY) || "[]");
            const actualizados = [nuevoRegistro, ...(Array.isArray(registros) ? registros : [])].slice(0, 100);
            localStorage.setItem(BITACORA_PRODUCTOS_KEY, JSON.stringify(actualizados));
            window.dispatchEvent(new Event("bitacoraActualizada"));
        } catch (error) {
            console.error("No se pudo registrar el producto en bitacora:", error);
        }
    };

    const obtenerCambiosProducto = (anterior, actualizado) => {
        const campos = [
            ["tipo", "Tipo"],
            ["nombre", "Nombre"],
            ...
            ["precio", "Precio"],
            ["marca", "Marca"],
            ["categoria_vehiculo", "Categoria vehiculo"],
        ...
        return campos
            .filter(([campo]) => String(anterior?.[campo] ?? "") !== String(actualizado?.[campo] ?? ""))
            .map(([campo, etiqueta]) => ({
                campo: etiqueta,
        ...
        if (res.isConfirmed) {
            try {
                ...
                registrarEventoProducto(
                    productoActual,
                    estaSuspendido ? "Producto reactivado" : "Producto suspendido",
                    estaSuspendido
                        ? `${productoActual.nombre || "El producto"} fue reactivado y vuelve a estar visible.`
                        : `${productoActual.nombre || "El producto"} fue suspendido y se oculto del catalogo.`,
                    [{
                        campo: "Estado",
                        anterior: estaSuspendido ? "Suspendido" : "Activo",
                        nuevo: estaSuspendido ? "Activo" : "Suspendido",
                    }]
                );
    ...
    const abrirModal = async (p = null) => {
        ...
        if (formValues) {
            ...
            try {
                ...
                if (p) {
                    const cambios = obtenerCambiosProducto(p, dataFinal);
                    registrarEventoProducto(
                        { ...p, ...dataFinal },
                        "Producto actualizado",
                        cambios.length > 0
                            ? `Se actualizaron ${cambios.length} campo(s) de ${dataFinal.nombre || p.nombre}.`
                            : `Se guardo ${dataFinal.nombre || p.nombre} sin cambios visibles.`,
                        cambios
                    );
                }

                // Avisamos a la bitacora que hubo un cambio de stock.
