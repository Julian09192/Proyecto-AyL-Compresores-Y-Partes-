...
app.post("/usuarios", (req, res) => {
  ...
  const password_hash = (req.body.password_hash || "").trim();
  const rol = req.body.rol || "cliente";

  if (!usuario || !correo || !password_hash) {
  ...
  db.query("SELECT id_usuario FROM usuario WHERE correo = ? LIMIT 1", [correo], (selectErr, existing) => {
    ...
    db.query(sql, [usuario, correo, password_hash, rol], (err, result) => {
      if (err) return res.status(500).json({ success: false, error: err.message });

      const token = jwt.sign(
        { id: result.insertId, rol, nombre: usuario },
        JWT_SECRET,
        { expiresIn: "24h" }
      );
...
/* 2. Ver historial de movimientos de un producto especÃ­fico */
...
/* 3. Buscar productos con stock crÃ­tico (Menos de 10 unidades) */
...
/* 4. Listar auditorÃ­a de cambios reciente */
...
/**
 * 1. OBTENER STOCK ESPECÃFICO (Path: ID)
 */
