...
// Agregamos setVista a las props para que funcione la navegación al checkout
function CartPanel({ carrito, cartOpen, setCartOpen, cambiarCantidad, eliminarDelCarrito, setVista }) {
  ...
  // CÁLCULO DEL TOTAL: Corregido para evitar el error .replace is not a function
  const total = carrito.reduce((acc, item) => {
    ...
    if (typeof item.precio === 'number') {
      ...
      // Si es un string (ej: "$2.500"), quitamos lo que no sea número y convertimos
      precioNumerico = parseInt(item.precio.replace(/\D/g, ""), 10) || 0;
    }
  ...
  const handleIrAlCheckout = () => {
    ...
    if (setVista) {
      setVista("checkout"); // Cambiamos la vista principal
    }
  };
    ...
    <>
      ...
      >
        ...
        <div className="d-flex justify-content-between align-items-center p-4 border-bottom">
          ...
          <button className="btn-close" onClick={() => setCartOpen(false)} aria-label="Close" />
        </div>
                ...
                <div key={item.id} className="d-flex gap-3 bg-light rounded-3 p-3">
                  ...
                  <div className="flex-grow-1 overflow-hidden">
                    ...
                    <div className="fw-bold" style={{ color: "#F5A623", fontSize: "0.88rem" }}>
                      {/* Formateamos el precio para mostrarlo siempre con puntos de miles */}
        ...
        {carrito.length > 0 && (
          <div className="p-4 border-top">
            ...
            <button
              ...
              style={{ background: "#F5A623", fontSize: "0.95rem", cursor: "pointer", transition: "0.3s" }}
