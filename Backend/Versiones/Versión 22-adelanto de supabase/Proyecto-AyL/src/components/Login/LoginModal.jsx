...
function LoginModal({ onClose, login }) {
  ...
  const API_AUTH_URL = "http://localhost:3001/usuarios"; // O la ruta base que use tu server.js
  ...
  const manejarEnvio = async (e) => {
    ...
    try {
      if (isLogin) {
        /* =========================================================
           INICIO DE SESIÓN DIRECTO CON SUPABASE
           ========================================================= */
        ...
        // Si tienes una ruta en Express para sincronizar perfiles y roles al loguear
        try {
        ...
        /* =========================================================
           REGISTRO ENRUTADO A TU BACKEND DE EXPRESS
           ========================================================= */
        ...
        // Llamamos a tu endpoint local en Node para procesar de forma segura
        const registroRes = await fetch(`${API_AUTH_URL}/register`, { // Modifica '/register' según diga tu authRoutes.js
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            email: email.trim(),
        ...
        // Capturamos el texto o JSON de respuesta de forma segura para evitar fallos de parsing
