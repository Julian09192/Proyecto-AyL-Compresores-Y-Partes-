...
// 1. Verificar si un email existe en la tabla usuarios
export const checkEmail = async (req, res) => {
  const { email } = req.body;
  if (!email) return res.status(400).json({ exists: false, error: "Email requerido" });

  try {
    // Sintaxis Supabase: busca en la tabla 'usuarios' donde 'correo' sea igual al email
...
// 2. Sincronizar usuario de Supabase Auth con tu tabla real 'usuarios'
...
// 3. Registrar un nuevo usuario en Supabase Auth e insertarlo en la tabla 'usuarios'
export const signUpUser = async (req, res) => {
  ...
  try {
    ...
    // B) Lo insertamos inmediatamente en tu tabla personalizada 'usuarios'
    const nombreUsuario = nombre || email.split('@')[0];
