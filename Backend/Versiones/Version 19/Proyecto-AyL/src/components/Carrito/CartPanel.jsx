...
function CartPanel({ carrito, cartOpen, setCartOpen, cambiarCantidad, eliminarDelCarrito, setVista, usuario, onOpenLogin }) {
  ...
  const total = carrito.reduce((acc, item) => {
    // Verificamos si item.precio es un string o un número para evitar errores
    const precioBase = typeof item.precio === 'string' 

  
      ? parseInt(item.precio.replace(/\D/g, ""), 10) 
      : item.precio;

    // Si por algún motivo no es un número válido, usamos 0
            ...
            <div className="d-flex flex-column gap-3">
              {carrito.map((item) => (
                <div key={item.id} className="d-flex gap-3 bg-light rounded-3 p-3">
                  ...
                  >
                    {/* Renderizamos emoji o una imagen si existe */}
                    {item.emoji || <i className="bi bi-box" style={{fontSize: '1rem'}}></i>}
                  </div>
                  ...
                  <div className="flex-grow-1 overflow-hidden">
                    ...
                    <div className="fw-bold" style={{ color: "#F5A623", fontSize: "0.88rem" }}>
                      ...
                      ${(typeof item.precio === 'number' ? item.precio : parseInt(item.precio?.replace(/\D/g, "") || 0)).toLocaleString("es-CO")}
                    </div>

                    <div className="d-flex align-items-center gap-2 mt-1">
                      <button
                        className="btn btn-sm border rounded-2 px-2 py-0 fw-bold bg-white"
        ...
        {carrito.length > 0 && (
          <div className="p-4 border-top bg-white">
            ...
            <button
              ...
              style={{ background: "#F5A623", fontSize: "0.95rem", cursor: "pointer", transition: "0.2s" }}
