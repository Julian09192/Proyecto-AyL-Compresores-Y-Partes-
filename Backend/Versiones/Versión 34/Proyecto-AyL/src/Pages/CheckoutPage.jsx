...
function CheckoutPage({ carrito, setVista, vaciarCarrito }) {
  ...
  // ==========================================
  // FUNCIÓN CORREGIDA - SOLO CAMPOS QUE EXISTEN
  // ==========================================
  const procesarTransaccionFinal = async () => {
    ...
    try {
      ...
      try {
        ...
        console.log("Usuario no autenticado, continuando con ID temporal");
      }

      if (!userId) {
        userId = `temp_${Date.now()}_${Math.random().toString(36).substring(7)}`;
      }

      const numeroOrden = `ORD-${Date.now()}-${Math.floor(Math.random() * 10000)}`;

      // 1. INSERTAR EN TABLA 'orden' - SOLO CAMPOS QUE EXISTEN
      const { data: pedidoInsertado, error: errorPedido } = await supabase
        .from("orden")
        .insert([
          {
            // SOLO estos campos existen en tu tabla según la imagen
            id_usuario_creacion: userId,
            // id_usuario: userId,  ← ELIMINADO porque no existe
      ...
      if (errorPedido) {
        console.error("Error detallado al insertar en orden:", errorPedido);
        throw new Error(`Error en orden: ${errorPedido.message}`);
      }

      console.log("Orden insertada exitosamente:", pedidoInsertado);

      // 2. PREPARAR ITEMS PARA 'orden_item'
      const productosPedidoPayload = carrito.map(item => ({
        id_orden: pedidoInsertado.id_orden,
        id_producto: item.id,
      ...
      console.log("Payload de items a insertar:", productosPedidoPayload);

      // 3. INSERTAR EN TABLA 'orden_item'
      const { error: errorProductos } = await supabase
        .from("orden_item")
        .insert(productosPedidoPayload);

      if (errorProductos) {
        console.error("Error detallado al insertar en orden_item:", errorProductos);
        throw new Error(`Error en items: ${errorProductos.message}`);
      }
      ...
      console.error("Error crítico guardando pedido en Supabase:", err);
      ...
      <div className="bg-light min-vh-100 d-flex align-items-center py-5 bootstrap-styles-fix text-center animate-fade-in">
        <div className="container">
          <div className="card border-0 shadow-sm p-5 rounded-4 bg-white mx-auto" style={{ maxWidth: '600px' }}>
            ...
            <button className="btn btn-warning w-100 py-3 fw-bold rounded-pill shadow-sm text-dark" onClick={() => { vaciarCarrito(); setVista("productos"); }}>
