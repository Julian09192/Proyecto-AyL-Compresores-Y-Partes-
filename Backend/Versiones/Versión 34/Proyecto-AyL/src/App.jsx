  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = onAuthStateChange(async (event, session) => {
      ...
      if ((event === 'SIGNED_IN' || event === 'INITIAL_SESSION') && session?.user) {
        ...
        if (perfilDb) {
          setUsuario(perfilDb);
          localStorage.setItem("usuario", JSON.stringify(perfilDb));
...
const login = (datosUsuario) => {
  // Asegurar que el ID se guarda completo
  const usuarioCompleto = {
    ...datosUsuario,
    id: datosUsuario.id || datosUsuario.id_orden || datosUsuario.id_usuario
  };
 
  setUsuario(usuarioCompleto);
  localStorage.setItem("usuario", JSON.stringify(usuarioCompleto));
  localStorage.setItem("al_login_fresco", "true");


  if (usuarioCompleto?.rol === "admin") setVista("admin");
  else if (usuarioCompleto?.rol === "empleado") setVista("empleado");
  else setVista("productos");
  ...
  const seoConfig = useMemo(() => {
    const pageMap = {
      ...
      politica: { title: "A&L Compresores y Partes | Política de Privacidad", description: "Política de privacidad de la empresa." }
    };
  ...
  function renderPagina() {
    switch (vista) {
      ...
      case "checkout": return <CheckoutPage carrito={carrito} setVista={setVista} vaciarCarrito={() => setCarrito([])} usuario={usuario} onOpenLogin={() => setMostrarModal(true)} />;
      case "contactos": return <ContactoPage {...propsComunes} />;
      case "perfil": return <ProfilePage usuario={usuario} setVista={setVista} volverA="inicio" />;
      ...
      case "politica": return <PoliticaPrivacidad {...propsComunes} />; // <-- AGREGADO AQUÍ PARA RENDERIZAR TU VISTA LEGAL
