...
// GET /api/notificaciones/conteo - NUEVA FUNCIÓN OPTIMIZADA
const obtenerConteoNoLeidas = async (req, res) => {
  try {
    // count: 'exact' y head: true le dicen a Supabase que no descargue los registros, solo devuelva el número total
    ...
    return res.status(200).json({ success: true, conteo: count || 0 });
  } catch (error) {
    console.error("Error en obtenerConteoNoLeidas:", error.message);
    return res.status(500).json({ error: "Error al obtener el conteo de notificaciones" });
  }
};
...
export const notificacionController = {
  ...
  obtenerConteoNoLeidas, // <-- Exportado correctamente aquí
