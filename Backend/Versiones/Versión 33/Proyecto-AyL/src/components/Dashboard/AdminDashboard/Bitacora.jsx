...
export default function Bitacora() {
  ...
  const cargarBitacora = async () => {
    ...
    setError(null);
    try {
      const res = await fetch('http://localhost:3001/api/bitacora');
  ...
  // 2. Efecto único para manejar el filtrado reactivo
  useEffect(() => {
  ...
  // Contadores optimizados en memoria
  ...
  // 3. Renderizador inteligente y RESPONSIVE de los detalles del cambio
  const renderizarDetalles = (detalles) => {
    ...
    if (detalles.includes('•')) {
      ...
      return (
        <div className="w-100">
          <div className="fw-semibold text-dark small mb-1 text-wrap text-break">{encabezado}</div>
          {/* El contenedor ahora usa flex-column en móviles y flex-row adaptable en pantallas más grandes */}
          <div className="d-flex flex-column flex-sm-row flex-wrap gap-1 mt-1">
            {listaCambios.map((cambio, index) => {
              if (cambio.includes('➔')) {
                const [campo, valores] = cambio.split(':');
                const [antes, despues] = valores.split('➔');
                return (
                  <span key={index} className="badge bg-white text-dark border d-flex d-sm-inline-flex align-items-center justify-content-between justify-content-sm-start flex-wrap p-2 shadow-sm my-1 text-wrap text-break" style={{ fontSize: '0.75rem', lineHeight: '1.2' }}>
                    <span className="me-1"><strong className="text-secondary">{campo.trim()}:</strong></span>
              ...
              return (
                <span key={index} className="badge bg-white text-dark border p-2 shadow-sm my-1 text-wrap text-break text-start d-block d-sm-inline-block" style={{ fontSize: '0.75rem' }}>
                  {cambio.trim()}
                </span>
              );
            })}
    ...
    if (detalles.includes('Suspendido') || detalles.includes('sacado de catálogo')) {
      return <span className="text-danger fw-medium small text-wrap text-break"><i className="bi bi-x-circle me-1"></i>{detalles}</span>;
    }
    if (detalles.includes('Reactivado') || detalles.includes('reincorporado')) {
      return <span className="text-info-emphasis fw-medium small text-wrap text-break"><i className="bi bi-check-circle me-1"></i>{detalles}</span>;
    }

    return <span className="text-muted small text-wrap text-break">{detalles}</span>;
  };
  ...
  return (
    <div className="container-fluid px-2 px-md-4 py-3 py-md-4">
      ...
      <div className="d-flex flex-column sm:flex-row justify-content-between align-items-start gap-3 mb-4 pb-3 border-bottom">
        <div>
          <h1 className="fw-bold mb-1" style={{ fontSize: "1.35rem", color: "#1e293b", letterSpacing: "-0.02em" }}>Bitácora de Movimientos</h1>
          <p className="text-muted small mb-0">Historial de cambios y auditoría general del sistema</p>
        </div>

        {/* Botón Sincronizar optimizado para móviles (w-100 en pantallas pequeñas) */}
        <button
          className="btn btn-warning fw-bold btn-sm d-flex align-items-center justify-content-center gap-2 px-3 py-2 shadow-sm text-dark w-100 w-sm-auto"
      ...
      {/* Botonera de Filtros con Contadores RESPONSIVE */}
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-stretch align-items-md-center mb-4 bg-white p-2 rounded-3 shadow-sm border gap-3">
        {/* Agregado un scroll horizontal nativo impecable táctil sin barras molestas */}
        <div className="d-flex overflow-auto pb-1 pb-md-0 gap-2 w-100 scrollbar-none" style={{ whiteSpace: 'nowrap', WebkitOverflowScrolling: 'touch' }}>
          <button 
            type="button" 
            className={`btn btn-sm px-3 rounded-2 fw-medium flex-grow-1 flex-md-grow-0 ${filtroActivo === 'TODOS' ? 'btn-dark' : 'btn-light border text-secondary'}`}
          ...
          <button 
            type="button" 
            className={`btn btn-sm px-3 rounded-2 fw-medium flex-grow-1 flex-md-grow-0 ${filtroActivo === 'INSERT' ? 'btn-success text-white' : 'btn-light border text-success'}`}
          ...
          <button 
            type="button" 
            className={`btn btn-sm px-3 rounded-2 fw-medium flex-grow-1 flex-md-grow-0 ${filtroActivo === 'UPDATE' ? 'btn-warning text-dark' : 'btn-light border text-warning-emphasis'}`}
          ...
          <button 
            type="button" 
            className={`btn btn-sm px-3 rounded-2 fw-medium flex-grow-1 flex-md-grow-0 ${filtroActivo === 'ESTADOS' ? 'btn-danger text-white' : 'btn-light border text-danger'}`}
            onClick={() => setFiltroActivo('ESTADOS')}
          >
            Estados <span className="badge bg-opacity-25 bg-dark ms-1">{contadores.estados}</span>
          </button>
        </div>
        <div className="text-muted small px-1 text-center text-md-end text-nowrap align-self-center">
          Mostrando <strong>{bitacoraFiltrada.length}</strong> de <strong>{bitacora.length}</strong>
        </div>
      </div>

      {error && (
        <div className="alert alert-danger rounded-3 mb-4 small shadow-sm text-wrap text-break" role="alert">
      ...
      {bitacoraFiltrada.length === 0 ? (
        <div className="text-center py-5 bg-white border rounded-3 shadow-sm px-3">
                  ...
                  <tbody style={{ fontSize: '0.85rem' }}>
                    {bitacoraFiltrada.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td className="py-3 pe-3 text-wrap text-break" style={{ maxWidth: '400px' }}>
              ...
              <div key={item.id} className="card shadow-sm border-1 rounded-3 bg-white">
                <div className="card-body p-3">
                  <div className="d-flex justify-content-between align-items-center mb-2 gap-2">
                    ...
                    <span className="text-muted font-monospace text-end" style={{ fontSize: '0.72rem' }}>
                  ...
                  {/* Detalle interno blindado contra desbordes */}
                  <div className="p-2 bg-light bg-opacity-50 border rounded-2 mb-2 w-100 text-wrap text-break overflow-hidden">
                    {renderizarDetalles(item.detalles)}
                  </div>

                  <div className="d-flex justify-content-between align-items-center mt-2 pt-2 border-top gap-2">
                    <span className="text-muted" style={{ fontSize: '0.72rem' }}>Ejecutado por:</span>
                    <span className="text-dark font-monospace fw-medium text-truncate text-end" style={{ fontSize: '0.75rem', maxWidth: '160px' }} title={item.usuario_email}>
                      {item.usuario_email || 'Sistema'}
                    </span>
                  </div>
                </div>
