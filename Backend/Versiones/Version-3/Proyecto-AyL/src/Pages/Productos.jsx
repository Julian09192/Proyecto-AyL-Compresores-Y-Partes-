...
function ProductCard({ producto, onAgregar }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Imagen con overlay del botón al hacer hover */}
        <div
        ...
        >
          ...
          {/* Overlay: botón "Añadir al carrito" estilo Falabella */}
          <div
            style={{
              position: "absolute",
              bottom: 0,
        ...
        <div className="card-body p-3" style={{ backgroundColor: "#fff" }}>
          ...
          <div className="d-flex align-items-center justify-content-between mt-2">
            ...
            {/* Botón siempre visible en móvil (sin hover) */}
            <button
...
function Productos({
  ...
  const [busqueda, setBusqueda] = useState("");            // ✅ FIX #1: estado declarado
  const [orden, setOrden] = useState("relevancia");       // ✅ FIX #2: estado de orden
  const [productosApi, setProductosApi] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);               // ✅ FIX: manejo de errores
  ...
  useEffect(() => {
    const obtenerProductos = async () => {
      try {
        ...
        setProductosApi(baseDatos.filter((p) => !p.suspendido));
      } catch (err) {
        console.error("Error al cargar productos:", err);
        setError("No pudimos cargar los productos. Verifica que json-server esté corriendo.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleAgregar = (producto) => {
    ...
    agregarAlCarrito({
      ...
      precio: Number(producto.Precio), // ✅ FIX #3: guardar número, no string
          ...
          <main className="col-lg-9">
            <div className="d-flex justify-content-between align-items-center mb-4 px-1 flex-wrap gap-2">
              ...
              <select
                ...
                value={orden}
                onChange={(e) => setOrden(e.target.value)}  // ✅ FIX #2
              >
            ...
            {!loading && !error && productosFiltrados.length === 0 && (
              <div className="text-center py-5">
                ...
                <button
                  className="btn btn-outline-warning rounded-pill mt-2"
