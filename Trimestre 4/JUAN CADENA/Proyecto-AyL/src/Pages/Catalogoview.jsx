...
// Función auxiliar limpia para parsear precios de forma segura (Definida arriba para evitar hoisting)
function inputPrecioFormato(valor) {
  return Number(valor || 0).toLocaleString('es-CO', { minimumFractionDigits: 0 });
}

function Catalogoview({ setVista, setProductoSeleccionadoId, agregarAlCarrito }) {
  ...
  // 🟢 FUNCIÓN INTERNA CORREGIDA: Envía el mapeo exacto de propiedades para el CartPanel
  const manejarAgregarAlCarrito = (e, producto) => {
    e.stopPropagation(); // Evita que al dar clic se abra la vista de detalle
    if (typeof agregarAlCarrito === "function") {
      agregarAlCarrito({
        id: producto.id,
        nombre: producto.nombre,
        precio: producto.precio,
        // Almacenamos la URL resolviendo primero el array de Supabase o la propiedad directa
        imagen_url: producto.producto_imagenes?.[0]?.imagen_url || producto.imagen_url || "",
        codigo_interno: producto.referencia_interna || producto.codigo_interno || "N/A",
        cantidad: 1
      });
    }
  };
  ...
  return (
    <div className="bg-light min-vh-100 d-flex flex-column pagina-catalogo-global">
            ...
            {loading ? (
              ...
              <div className="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-xl-4 g-4 animate-fade-in-up">
                {productosFiltrados.map((producto) => {
                  const imagenFinal = producto.producto_imagenes?.[0]?.imagen_url || producto.imagen_url || "https://placehold.co/400x400?text=Sin+Imagen";
                  return (
                        ...
                        <div className="card-body d-flex flex-column justify-content-between p-3 pt-2">
                          ...
                          <div className="pt-2 border-top border-light d-flex align-items-center justify-content-between mt-auto">
                            <div className="d-flex flex-column">
                              ...
                              <span className="fs-6 fw-extrabold text-dark mt-1">
                                ${inputPrecioFormato(producto.precio)}
                              </span>
                            </div>
                            
                            {/* Botón añadido para enviar datos correctos */}
                            <button 
                              className="btn btn-warning btn-sm rounded-circle p-0 d-flex align-items-center justify-content-center shadow-sm text-dark hover-scale"
                              style={{ width: "32px", height: "32px" }}
                              onClick={(e) => manejarAgregarAlCarrito(e, producto)}
                    ...
                    >
                      <div className="d-flex flex-column flex-sm-row align-items-center gap-3">
                        ...
                        <div className="text-center text-sm-end flex-shrink-0 mt-2 mt-sm-0 ps-sm-3 d-flex flex-row flex-sm-column align-items-center align-items-sm-end gap-3 gap-sm-0">
                          ...
                          {/* Botón añadido para enviar datos correctos */}
                          <button 
                            className="btn btn-warning btn-sm rounded-pill px-3 fw-bold align-self-center shadow-sm text-dark hover-scale"
                            onClick={(e) => manejarAgregarAlCarrito(e, producto)}
                          >
      ...
      <style>{`
        .pagina-catalogo-global { letter-spacing: -0.1px; }
